...
- **Phase 0.5 is complete** — see [ithkuil-validation-report.md](ithkuil-validation-report.md). Verdict: the font is faithful to the 2011 script wherever a reference exists (primary 24/24, tertiary 7/7, 11 consonants); only 2 genuine glyph discrepancies (`k'`, `y`); ~55 glyphs have no isolated reference (diacritics, numerals, font-only secondaries) and are *not* errors.

## Direction (DECIDED)

The project targets **New Ithkuil, bidirectional**. The ithkey 2011 font is now a *reference/harness*, not the core:
- **Forward (text → New Ithkuil script SVG):** reuse [`@zsnout/ithkuil`](https://github.com/zsakowitz/ithkuil) (MIT, TypeScript, npm `@zsnout/ithkuil`). Its `/script` module composes primary/secondary/tertiary/quaternary characters + diacritics algorithmically. Caveat: SVG gen uses the DOM (`document.createElementNS`), so Node use needs a shim like `linkedom`. Do NOT reverse-engineer a font for the forward path.
- **Synthetic data:** the forward pipeline renders labeled glyph images (augmented) to train/eval the reverse pipeline.
- **Reverse (script image → text):** the novel, unsolved part — this is the project's actual contribution. `@zsnout/ithkuil` is text→script ONLY (no OCR). Pipeline: preprocess → segment → classify (baseline = the Phase 0.5 similarity/template-match harness in `build_glyph_similarity.py`) → reconstruct word JSON → `@zsnout/ithkuil` `/generate` → romanized text.
- The Phase 0/2011 artifacts (glyph inventory, mapping table, validator) are 2011-script and do NOT carry over to New Ithkuil glyph shapes; keep them as reference only.
